...
{
    const iframe = window.frames['menu'];
...
{
    const extentHeightPc = 190;
    const extentHeightMobile = 400;
    const iframe = document.getElementById('menu');
    let height = iframe.scrollHeight;
    // let width = window.screen.width;
    // let aWidth = window.screen.availWidth;
    let eleWidth = iframe.scrollWidth;
    // const element = iframe.contentWindow.document.getElementById('mobile_menu');
    // const className = element.className;
    interval = null;

    if (eleWidth >= 768)
    {
        iframe.style.height = height + extentHeightPc + 'px';
    }
    else
    {
        // iframe.style.height = height + 400 + 'px';
        interval = setInterval(() => {
            const iframe = document.getElementById('menu');
            const className = iframe.contentWindow.document.getElementById('mobile_menu').className;
...
function resetIframe()
{
    ...
    const className = element.className;
    let eleWidth = iframe.scrollWidth;
    if (className == 'hidden' || eleWidth >= 768)
    {
        iframe.style.height = '64px';
    }
}